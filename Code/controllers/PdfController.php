...
class PdfController {
    ...
    public function downloadpdf(){
        ...
        $id_nomina =isset($_REQUEST['id_nomina']) ? $_REQUEST['id_nomina'] : false;
        $user =isset($_REQUEST['id']) ? $_REQUEST['id'] : false;
        
        if($id_nomina || $user || $token)
        {
            
            if($user && $_SESSION['ADMINISTRADOR']->fk_rol == 1 )
            {
                $conceptos=Nomina::consultarConceptosPorNomina($id_nomina);
            }else{
                
                $conceptos=Nomina::consultarConceptosPorNominaToken($token,$id_nomina);
            }
