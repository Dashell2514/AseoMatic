...
class Login extends DataBase{
    public function verificarLogin($email)
    {
        try {
            ...
            $stm = parent::conectar()->prepare("SELECT id_usuario,nombres,apellidos,correo,clave,img_usuario,numero_documento,fk_rol,fk_cargo,fk_tipo_documento,fk_tipo_contrato,token,created_at,updated_at,nombre_rol FROM usuarios INNER JOIN roles ON usuarios.fk_rol= roles.id_rol INNER JOIN tipos_documentos ON usuarios.fk_tipo_documento=tipos_documentos.id_tipo_documento AND usuarios.correo=? ");
